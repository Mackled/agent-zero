...
Click to open a video to learn how to install Ayla Agent:

[![Easy Installation guide](/docs/res/easy_ins_vid.png)](https://www.youtube.com/watch?v=w5v5Kjx51hs)

The following user guide provides instructions for installing and running Ayla Agent using Docker, which is the primary runtime environment for the framework. For developers and contributors, we also provide instructions for setting up the [full development environment](#in-depth-guide-for-full-binaries-installation).
...
- Docker Desktop provides the runtime environment for Ayla Agent, ensuring consistent behavior and security across platforms
...
- Note: Ayla Agent also offers a Hacking Edition based on Kali linux with modified prompts for cybersecurity tasks. The setup is the same as the regular version, just use the aylaagent/ayla-agent:hacking image instead of aylaagent/ayla-agent.

2.1. Pull the Ayla Agent Docker image:
- Search for `aylaagent/ayla-agent` in Docker Desktop
...
- Choose or create a directory on your machine where you want to store Ayla Agent's data
- This can be any location you prefer (e.g., `C:/ayla-agent-data` or `/home/user/ayla-agent-data`)
- This directory will contain all your Ayla Agent files, like the legacy root folder structure:
  ...
  - `.env` - Your API keys
  - `settings.json` - Your Ayla Agent settings

> [!TIP]
> Choose a location that's easy to access and backup. All your Ayla Agent data 
> will be directly accessible in this directory.

2.3. Run the container:
- In Docker Desktop, go back to the "Images" tab
- Click the `Run` button next to the `aylaagent/ayla-agent` image
...
- Under "Volumes", configure:
  - Host path: Your chosen directory (e.g., `C:\ayla-agent-data`)
  - Container path: `/ayla`
...
> Alternatively, run the following command in your terminal:
> ```bash
> docker run -p $PORT:80 -v /path/to/your/data:/ayla aylaagent/ayla-agent
> ```
...
- The Web UI will open. Ayla Agent is ready for configuration!

![docker ui](res/setup/6-docker-AYLA-running.png)

> [!TIP]
> You can also access the Web UI by clicking the ports right under the container ID in Docker Desktop.

> [!NOTE]
> After starting the container, you'll find all Ayla Agent files in your chosen 
...
3. Configure Ayla Agent
- Refer to the following sections for a full guide on how to configure Ayla Agent.

## Settings Configuration
Ayla Agent provides a comprehensive settings interface to customize various aspects of its functionality. Access the settings by clicking the "Settings"button with a gear icon in the sidebar.
...
Learn more about Remote Function Calls and their purpose [here](#7-configure-ayla-agent-rfc).

> [!IMPORTANT]
> Always keep your API keys and passwords secure.

# Choosing Your LLMs
The Settings page is the control center for selecting the Large Language Models (LLMs) that power Ayla Agent.  You can choose different LLMs for different roles:

| LLM Role | Description |
| --- | --- |
| `chat_llm` | This is the primary LLM used for conversations and generating responses. |
| `utility_llm` | This LLM handles internal tasks like summarizing messages, managing memory, and processing internal prompts.  Using a smaller, less expensive model here can improve efficiency. |
| `embedding_llm` | This LLM is responsible for generating embeddings used for memory retrieval and knowledge base lookups. Changing the `embedding_llm` will re-index all of AYLA's memory. |
...
> mixed in the vector database. Note that this will DELETE ALL of Ayla Agent's memory.
...
#### Selecting your model within Ayla Agent
...
## Using Ayla Agent on your mobile device
Ayla Agent's Web UI is accessible from any device on your network through the Docker container:
...
> [!NOTE]
> If you're running Ayla Agent directly on your system (legacy approach) instead of 
> using Docker, you'll need to configure the host manually in `run_ui.py` to run on all interfaces using `host="0.0.0.0"`.

For developers or users who need to run Ayla Agent directly on their system,see the [In-Depth Guide for Full Binaries Installation](#in-depth-guide-for-full-binaries-installation).

# How to update Ayla Agent

1. **If you come from the previous version of Ayla Agent:**
- Your data is safely stored across various directories and files inside the Ayla Agent folder.
- To update to the new Docker runtime version, you might want to backup the following files and directories:
  ...
  - `/tmp/settings.json` - Your Ayla Agent settings
...
- Reach for the folder where you saved your data and copy it to the new Ayla Agent folder set during the installation process.
- Ayla Agent will automatically detect your saved data and use it across memory, knowledge, instruments, prompts and settings.

> [!IMPORTANT]
> If you have issues loading your settings, you can try to delete the `/tmp/settings.json` file and let Ayla Agent generate a new one.
...
- Go to "Images" tab and remove the `aylaagent/ayla-agent` image or click the three dots to pull the difference and update the Docker image.
...
> your Ayla Agent data directory (the chosen directory on your machine).
...
> docker stop ayla-agent
>
> # Remove the container (data is safe in the folder)
> docker rm ayla-agent
>
> # Remove the old image
> docker rmi aylaagent/ayla-agent
>
...
3. **Full Binaries**
- Using Git/GitHub: Pull the latest version of the Ayla Agent repository. 
...
> - If any changes are made to the requirements of the updated version, you have to execute this command inside the AYLA conda env after activating it:
> ```bash
> pip install -r requirements.txt

# In-Depth Guide for Full Binaries Installation
- Ayla Agent is a framework. It's made to be customized, edited, enhanced. Therefore you need to install the necessary components to run it when downloading its full binaries. This guide will help you to do so.
- The following step by step instructions can be followed along with a video for this tutorial on how to make Ayla Agent work with its full development environment.
...
2. You don't necessarily need API keys: Ayla Agent can run with local models. For this tutorial though, we will leave it to the default OpenAI API. A guide for downloading Ollama along with local models is available [here](#installing-and-using-ollama-local-models).
...
6. Running Ayla Agent without Docker makes the process more complicated and it's thought for developers and contributors.
...
## 2. Download Ayla Agent
- You can clone the Ayla Agent repository (https://github.com/aylaagent/ayla-agent) from GitHub if you know how to use Git. In this tutorial I will just show how to download the files.

1. Go to the Ayla Agent releases [here](https://github.com/aylaagent/ayla-agent/releases).
2. The latest release is on the top of the list, click the "Source Code (zip)" button under "Assets" to download it.

<img src="res/setup/image-14-u.png" alt="Ayla Agent download" width="500"/>
<br><br>

3. Extract the downloaded archive where you want to have it. I will extract them to "ayla-agent" folder on my Desktop - "C:\Users\frdel\Desktop\ayla-agent" on Windows and "/Users/frdel/Desktop/ayla-agent" on macOS.
...
2. In the terminal, navigate to your Ayla Agent folder using **"cd"** command. Replace the path with your actual Ayla Agent folder path.
~~~
cd C:\Users\frdel\Desktop\ayla-agent
~~~
You should see your folder has changed on the next terminal line.

<img src="res/setup/image-15.png" alt="Ayla Agent cd" height="100"/>
<img src="res/setup/image-16.png" alt="Ayla Agent cd" height="100"/>
<br><br>

3. Create Conda environment using command **"conda create"**. After **"-n"** is your environment name, you can choose your own, i will use **"AYLA"** - short for Ayla Agent. After **"python"** is the Python version that Conda will install for you into this environment, right now, 3.12 works fine. **-y** skips confirmations.
~~~
conda create -n AYLA python=3.12 -y
~~~
...
conda activate AYLA
~~~
And you should see that the **(base)** on the left has changed to **(AYLA)**. This means that this terminal now uses the new **AYLA** virtual environment and all packages will be installed into this environment.
...
> "conda activate AYLA" again for that window.
...
Simply put, Docker is a way of running virtual computers on your machine. These are lightweight, disposable and isolated from your operating system, so it is a way to sandbox Ayla Agent.
- Ayla Agent only connects to the Docker container when it needs to execute code and commands. The frameworks itself runs on your machine.
...
> Docker socket to be used (requires password)."  This allows Ayla Agent to 
...
- Ayla Agent needs a Docker image to be pulled from the Docker Hub to be run, even when using the full binaries.
You can refer to the [installation instructions above](#windows-macos-and-linux-setup-guide) to run the Docker container and then resume from the next step. There are two differences:
  ...
  - You need to map the `/ayla` volume to the location of your local Ayla Agent folder.
- Run the Docker container following the instructions.

## 5. Run the local Ayla Agent instance
Run the Ayla Agent with Web UI:
~~~
python run_ui.py
~~~
...
- Open the URL shown in terminal in your web browser. You should see the Ayla Agent interface.

## 6. Configure Ayla Agent
Now we can configure Ayla Agent - select models, settings, API Keys etc. Refer to the [Usage](usage.md#agent-configuration) guide for a full guide on how to configure Ayla Agent.

## 7. Configure Ayla Agent RFC
Ayla Agent needs to be configured further to redirect some functions to the Docker container. This is crucial for development as AYLA needs to run in a standardized environment to support all features.
...
9. Now you have the full development environment to work on Ayla Agent.
...
### Conclusion
After following the instructions for your specific operating system, you should have Ayla Agent successfully installed and running. You can now start exploring the framework's capabilities and experimenting with creating your own intelligent agents. 

If you encounter any issues during the installation process, please consult the [Troubleshooting section](troubleshooting.md) of this documentation or refer to the Ayla Agent [Skool](https://www.skool.com/ayla-agent) or [Discord](https://discord.gg/B8KZKNsPpj) community for assistance.
