...
//     if (out.name.startsWith("data-") && out.name != "data-AYLAUID" && out.name != "data-AYLA-frame-id") return null;
...
//   window.__AYLA_extractFrameContent = function() {
...
//           node.setAttribute("data-AYLAUID", uid);
//         }
...
//         if (tagName) {
//           content += ` selector="${node.getAttribute("data-AYLAUID")}"`;
//         }
...
//     if (event.data === 'AYLA_REQUEST_CONTENT') {
//       // Extract content and send it back to parent
//       const content = window.__AYLA_extractFrameContent();
//       // Use '*' as targetOrigin since we're in a controlled environment
//       window.parent.postMessage({
//         type: 'AYLA_FRAME_CONTENT',
//         content: content,
//         frameId: window.frameElement?.getAttribute('data-AYLA-frame-id')
//       }, '*');
//     }
//   });

//   // Function to extract content from all frames
//   window.__AYLA_extractAllFramesContent = async function(rootNode = document) {
//     let content = "";
    
//     // Extract content from current document
//     content += window.__AYLA_extractFrameContent();
...
//         iframe.setAttribute('data-AYLA-frame-id', frameId);
        
//         // Setup one-time message listener for this specific frame
//         const listener = function(event) {
//           if (event.data?.type === 'AYLA_FRAME_CONTENT' && 
...
//         iframe.contentWindow.postMessage('AYLA_REQUEST_CONTENT', '*');
...
//       const frameId = iframe.getAttribute('data-AYLA-frame-id');
