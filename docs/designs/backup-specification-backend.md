# Ayla Agent Backup/Restore Backend Specification

## Overview
This specification defines the backend implementation for Ayla Agent's backup and restore functionality, providing users with the ability to backup and restore their Ayla Agent configurations, data, and custom files using glob pattern-based selection. The backup functionality is implemented as a dedicated "backup" tab in the settings interface for easy access and organization.
...
- Integrates with Ayla Agent's established error handling and toast notification system
...
backup_section: SettingsSection = {
    ...
    "description": "Backup and restore Ayla Agent data and configurations using glob pattern-based file selection.",
    "fields": [
        {
            "id": "backup_create",
            "title": "Create Backup",
...
def _get_default_patterns(self) -> str:
    ...
    agent_root = self.ayla_agent_root.rstrip('/')
    user_home = self.user_home.rstrip('/')

    return f"""# Ayla Agent Knowledge (excluding defaults)
{agent_root}/knowledge/**
!{agent_root}/knowledge/default/**

# Ayla Agent Instruments (excluding defaults)
...
/ayla/knowledge/**
!/ayla/knowledge/default/**
/root/**
!/root/.*/**
!/root/.*

# Host environment
/home/rafael/ayla/data/knowledge/**
!/home/rafael/ayla/data/knowledge/default/**
/home/rafael/**
!/home/rafael/.*/**
!/home/rafael/.*
```

> **⚠️ CRITICAL FILE NOTICE**: The `{agent_root}/.env` file contains essential configuration including API keys, model settings, and runtime parameters. This file is **REQUIRED** for Ayla Agent to function properly and should always be included in backups alongside `settings.json`. Without this file, restored Ayla Agent instances will not have access to configured language models or external services.
...
class BackupCreate(ApiHandler):
    ...
    async def process(self, input: dict, request: Request) -> dict | Response:
        patterns = input.get("patterns", "")
        include_hidden = input.get("include_hidden", False)
        backup_name = input.get("backup_name", "ayla-agent-backup")
    ...
    async def process(self, input: dict, request: Request) -> dict | Response:
        ...
        try:
            ...
            return {
                ...
                "ayla_agent_version": metadata.get("ayla_agent_version", "unknown"),
...
The BackupService leverages Ayla Agent's existing file operation helpers which already support RFC (Remote Function Call) routing for development mode. This ensures seamless operation whether running in direct mode or with container isolation.
...
class BackupService:
    """Core backup and restore service for Ayla Agent"""

    def __init__(self):
        self.ayla_agent_version = self._get_ayla_agent_version()
        self.ayla_agent_root = files.get_abs_path("")  # Resolved Ayla Agent root
    ...
    def _get_ayla_agent_version(self) -> str:
        """Get current Ayla Agent version"""
        try:
    ...
    async def _get_environment_info(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                "ayla_agent_root": files.get_abs_path(""),
    ...
    async def create_backup(self, patterns: str, include_hidden: bool = False, backup_name: str = "ayla-agent-backup") -> str:
        ...
        try:
            with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
                ...
                metadata = {
                    ...
                    "ayla_agent_version": self.ayla_agent_version,
    ...
    def create_backup_with_progress(self, patterns: str, include_hidden: bool = False, backup_name: str = "ayla-agent-backup"):
...
#### Ayla Agent Internal Dependencies
The backup system requires these Ayla Agent helper modules:
...
#### Integration with Ayla Agent Error System
The backup system integrates with Ayla Agent's existing error handling infrastructure:

```python
from python.helpers.errors import format_error
from python.helpers.print_style import PrintStyle

# Follow Ayla Agent's error handling patterns
try:
...
- Restrict backups to predefined base directories (/ayla, /root)
...
BACKUP_CONFIG = {
    ...
    "allowed_base_paths": ["/ayla", "/root"]
}
```

#### Future Integration Opportunities
**Task Scheduler Integration:**
Ayla Agent's existing task scheduler could be extended to support automated backups:
...
The backup system uses the same version detection method as Ayla Agent's main UI:
...
```json
{
  ...
  "include_patterns": [
    ...
    "/ayla/memory/**",
    "/ayla/.env",
    "/ayla/tmp/settings.json"
  ],
  "exclude_patterns": [
    "/ayla/knowledge/default/**",
    "/ayla/instruments/default/**",
    "/ayla/memory/embeddings/**"
  ],
...
- **Environment Details**: User, timezone, working directory, runtime mode, Ayla Agent root path
...
- **Compatibility Data**: Ayla Agent version and environment for restoration validation
...
This enhanced backend specification provides a production-ready, comprehensive backup and restore system with advanced metadata tracking, real-time progress monitoring, and intelligent file management capabilities, all while maintaining Ayla Agent's architectural patterns and security standards.
...
- **Testing Verified**: BackupService initializes correctly and detects Ayla Agent root paths
...
**Problem:** When `include_hidden=false`, the system was excluding ALL hidden files, even when they were explicitly specified in patterns like `/ayla/.env`.

**Solution:** Implemented explicit pattern detection that distinguishes between:
- **Explicit patterns** (like `/ayla/.env`) - Always included regardless of `include_hidden` setting
- **Wildcard discoveries** (like `/ayla/*`) - Respect the `include_hidden` setting

**Result:** Critical files like `.env` are now properly backed up when explicitly specified, ensuring Ayla Agent configurations are preserved.
...
The Ayla Agent backup system is now production-ready and battle-tested! 🚀
...
backed_up_agent_root = environment_info.get("ayla_agent_root", "")
```
...
- Works seamlessly between different Ayla Agent installations
...
The Ayla Agent backup system now provides:
