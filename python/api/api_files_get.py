        ...
        try:
            ...
            for path in paths:
                try:
                    ...
                    if path.startswith("/ayla/tmp/uploads/"):
                        # Internal path - convert to external
                        filename = path.replace("/ayla/tmp/uploads/", "")
                        external_path = files.get_abs_path("tmp/uploads", filename)
                        filename = os.path.basename(external_path)
                    elif path.startswith("/ayla/"):
                        # Other internal Ayla Agent paths
                        relative_path = path.replace("/ayla/", "")
